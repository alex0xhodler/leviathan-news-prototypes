...
**Goal:** Port the three validated prototype experiences (Terminal / Broadsheet / Abyssal) into the Leviathan app behind feature flag `lev_v_3` — disabled by default, internal testers first — covering **both the homepage/feed view and the article view**, with a first-visit style picker and full Mixpanel instrumentation for a 3-way style A/B test against the existing flow.

**Architecture:** A device-scoped style preference (`terminal | broadsheet | abyssal | classic`) resolves the theme for **both the home/feed route and the article route** — the picker chooses a coherent end-to-end experience, not a per-page skin. All themes share one behavioral contract and one data layer; only presentation differs. The article contract is the "hook journey" (§3); the homepage contract is the themed feed (§4, per-theme homepage specs). Entry points change globally under the flag: headline taps open the **internal article view** instead of exiting to source.
...
- When `lev_v_3` is on and the device has **no stored preference**, show the picker **once per device** on first visit to **either the homepage or an article** (bottom sheet on mobile, modal on desktop).
...
- Persist choice in `localStorage["lev_style"]`. Device-scoped, not account-scoped (deliberate: matches prototype behavior and keeps anonymous users in the experiment).
...
### Homepage contract (all themes)
- Themed feed of the same live data (`GET /news/?status=approved&sort_type=hot|new|top`), sort switcher, species attribution (6px dots: gray human / lime cyborg / navy agent), ▲ SQUID weight per story, relative timestamps.
- **Headline taps open the internal article view** — external exit exists only inside the article page (see §3 zone 7).
- Live update affordance (WebSocket `wss://api.leviathannews.xyz/ws/news/`): new items announce, never jump the list under the user's thumb.
- Skeleton shimmer loading; error state with retry; empty-feed copy.
...
- **Homepage:** 40px ticker strip (brand + scrolling trending-narratives marquee + platform pulse counters 1H/6H/24H + WS live dot — on mobile: brand + live dot ONLY, marquee/pulse/stale chip removed); feed = dense ledger rows (rank numeral · headline · source + tag mono text · species dot · time · ▲weight), row tap expands an inline drawer (TLDR, top yap, actions incl. "OPEN FULL DISPATCH →"); HOT/NEW/TOP/WEIGHT switcher — on mobile the switcher is the ONLY feed header (4 equal 40px tabs full width; feed name, count, key hints hidden); narrative filter rail desktop-only. Reference: `variant-1-terminal.html`.
...
- **Homepage:** WSJ front page — nameplate masthead + dateline, one lead story (large serif headline, standfirst, figure), remaining wire as ruled story rows with serif headlines and mono meta; light/dark toggle persists; sort switcher as a thin ruled tab row. Mobile: compact one-row masthead, lead story condensed, hairline-separated rows. Reference: `variant-2-editorial.html`.
...
- **Homepage:** SURFACE SIGNAL hero (top story in a hairline panel with depth tag) + remaining feed as dark glass ledger rows; AGENT PROTOCOL hairline panel for the agent/API pitch; bioluminescent accents limited to hairlines, dots, and the live pulse; depth readout in the header follows scroll. Reference: `variant-3-abyssal.html`.
...
2. `lev_style` storage + first-visit picker sheet/modal (4 tiles) + switcher entry point.
...
4b. Themed homepage/feed components ×3 against the homepage contract + per-theme homepage specs in §4, sharing the same feed controller and data layer; sort switcher wired to the same API params as today.
...
8. QA matrix: 3 themes × {homepage, article} × mobile/desktop × anonymous/authed × no-TLDR article (brief hides) × no-yaps article (empty pod) × WS offline (fixture/error state) × style switch mid-session.
...
- `prototypes/variant-1-terminal.html` / `variant-2-editorial.html` / `variant-3-abyssal.html` — homepage/feed themes
...
**Untouched scope (do not assume done):** the actual Leviathan app repo (nothing there has been modified — zero commits); variants 4/5/6; real prediction-market odds (no API exists — consensus-as-position is the wedge); backend work of any kind.
...
**Must not assume:** that the prototypes repo's aesthetic approval equals approval to change app flows outside the homepage + article views, and that the Leviathan API offers a prediction-market endpoint (it does not — use the consensus wedge + provider seam).
